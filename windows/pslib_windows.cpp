...
DiskPartitionInfo* disk_partitions(bool all)
{
    ...
    while (*drive_letter != '\0') {
        ...
        if (all == 0) {
            if ((type == DRIVE_UNKNOWN) || (type == DRIVE_NO_ROOT_DIR) || (type == DRIVE_REMOTE) || (type == DRIVE_RAMDISK)) {
                goto next;
            }
...
UsersInfo* get_users()
{
    ...
    for (i = 0; i < count; i++) {
        ...
        check(WTSQuerySessionInformation(hServer, sessionId, WTSClientAddress,
                  &buffer_addr, &bytes),
            "error in retriving ClientAddress")

            address
            = (PWTS_CLIENT_ADDRESS)buffer_addr;
...
CpuTimes* calculate(CpuTimes* t1, CpuTimes* t2)
{
    ...
    check_mem(ret);
    all_delta = (t2->user + t2->system + t2->interrupt + t2->idle + t2->dpc) - (t1->user + t1->system + t1->interrupt + t1->idle + t1->dpc);
    if (all_delta == 0)
        all_delta = 100;
...
uint32_t get_boot_time()
{
    ...
    psutil_GetTickCount64 = (__psutil_GetTickCount64)GetProcAddress(hKernel32, "GetTickCount64");
...
CpuTimes* per_cpu_times()
{
    ...
    sppi = (_SYSTEM_PROCESSOR_PERFORMANCE_INFORMATION*)
        malloc(si.dwNumberOfProcessors * sizeof(_SYSTEM_PROCESSOR_PERFORMANCE_INFORMATION));
    ...
    status = NtQuerySystemInformation(
        ...
        sppi,
        si.dwNumberOfProcessors * sizeof(_SYSTEM_PROCESSOR_PERFORMANCE_INFORMATION),
        NULL);
    ...
    for (i = 0; i < si.dwNumberOfProcessors; i++) {
        ...
        interrupt = (float)((HI_T * sppi[i].InterruptTime.HighPart) + (LO_T * sppi[i].InterruptTime.LowPart));
        dpc = (float)((HI_T * sppi[i].DpcTime.HighPart) + (LO_T * sppi[i].DpcTime.LowPart));
...
cpustats* cpu_stats()
{
    ...
    spi = (_SYSTEM_PERFORMANCE_INFORMATION*)
        malloc(si.dwNumberOfProcessors * sizeof(_SYSTEM_PERFORMANCE_INFORMATION));
    check_mem(spi);
    status = NtQuerySystemInformation(
...
int pid_in_pids(DWORD pid)
{
    DWORD* proclist = NULL;
    DWORD numberOfReturnedPIDs;
    DWORD i;

    proclist = (DWORD*)pids((uint32_t*)&numberOfReturnedPIDs);
    if (proclist == NULL)
        return -1;
...
// read the CommandLine UNICODE_STRING structure
    ...
    check(ReadProcessMemory(hProcess, (PCHAR)rtlUserProcParamsAddress + 112,
        ...
        "Could not to read Command Line Unicode Structure");
#else
    check(ReadProcessMemory(hProcess, (PCHAR)rtlUserProcParamsAddress + 0x40,
              &commandLine, sizeof(commandLine), NULL),
        "Could not read Command Line Unicode Structure");
#endif
    ...
    check(ReadProcessMemory(hProcess, commandLine.Buffer,
        ...
        "Failed to read the commandline");
...
char* Username(pid_t pid)
{
    ...
    if (pid == 0 || pid == 4) {
        ...
        // Get the user SID.
        while (1) {
            user = (PTOKEN_USER)malloc(bufferSize);
            check_mem(user);
        ...
        // resolve the SID to a name
        while (1) {
            name = (WCHAR*)malloc(nameSize * sizeof(WCHAR));
            check_mem(name);
            domainName = (WCHAR*)malloc(domainNameSize * sizeof(WCHAR));
...
int get_proc_info(DWORD pid, PSYSTEM_PROCESS_INFORMATION* retProcess,
    PVOID* retBuffer)
{
...
error:
    ...
    log_err("%s", err);
    free(err);
    return static_cast<proc_status>(20);
}
...
enum proc_priority nice(pid_t pid)
{
    ...
    CloseHandle(hProcess);
    return static_cast<proc_priority>(priority);
...
double create_time(pid_t pid)
{
    ...
    hProcess = handle_from_pid_waccess(pid, PROCESS_QUERY_INFORMATION | PROCESS_VM_READ);
    if ((hProcess == NULL) && ((GetLastError() == ERROR_ACCESS_DENIED) || (errno == EPERM) || (errno == EACCES))) {
    ...
    if (!GetProcessTimes(hProcess, &ftCreate, &ftExit, &ftKernel, &ftUser)) {
        if ((GetLastError() == ERROR_ACCESS_DENIED) || (errno == EPERM) || (errno == EACCES)) {
            if (get_proc_info(pid, &process, &buffer)) {
                goto find_time;
            } else {
